...
script_author("1NS")
script_description("Notifies in which city and district there is a gangwar")

require 'lib.moonloader'
local sampev = require 'lib.samp.events'
local gangZonesTable = {}

function sampev.onCreateGangZone(zoneId, squareStart, squareEnd, color)
    -- Fill gangzones table on player connect
    gangZonesTable = {
	   {
	   zoneid = zoneId, 
	   squarestartx = squareStart.x,
	   squarestarty = squareStart.y,
	   squareendx = squareEnd.x,
	   squareendy = squareEnd.y,
	   zonecolor = color
	   },
	}
end
...
function sampev.onGangZoneFlash(zoneid, color)
   for key, value in pairs(gangZonesTable) do
      if zoneid == value.zoneid then
         sampAddChatMessage(string.format("Gangwar on %s - %s - zone %i", 
		 calculateCity(value.squarestartx, value.squarestarty, 0.0),
		 calculateZone(value.squarestartx, value.squarestarty, 0.0),
		 zoneid),
		 0x00FF0000)
	  end
   end
...
function sampev.onGangZoneStopFlash(zoneid)
   sampAddChatMessage(string.format("Gangwar finished on zone %i", zoneid), 0x0000FF00)
end

-- SAMP UDF Based functions

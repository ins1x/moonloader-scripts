...
script_url("https://github.com/ins1x/moonloader-scripts/tree/main/renderobjects")
script_version("0.3")
...
local checkboxShowDistance = imgui.ImBool(false)
local checkboxRenderIDsByRule = imgui.ImBool(false)
local checkboxRenderDistanceByRule = imgui.ImBool(true)
...
function main()
    ...
    local font = renderCreateFont("Arial", 6, 5)
    ...
    while true do
        ...
        -- Hide render on F10 key
        if isKeyJustPressed(0x79) and not isSampfuncsConsoleActive() then
           checkboxRenderAll.v = false
           checkboxRenderByRule.v = false
        end
        ...
        if checkboxRenderAll.v or checkboxRenderByRule.v then
            for index, v in pairs(getAllObjects()) do
                ...
                if isObjectOnScreen(v) and not isPauseMenuActive() then
                    ...
                    local color = string.format("0xFF%s", rgbToHex(index+32, index+64, index+128))
					
					-- mindistance used to ignore player attached objects	
                    if checkboxRenderAll.v and sampObjectModelNames[model] ~= nil 
                    and distance > mindistance then
                        if checkboxShowDistance.v then 
                            renderFontDrawText(
                                font,
                                (checkboxShowIDs.v and objectid 
                                and "model: " .. sampObjectModelNames[model] .. "(" .. model .. ") ; id: " .. objectid
                                or "model: " .. sampObjectModelNames[model] .. "(" .. model .. ")") .. "; distance: "
                            ...
                            renderFontDrawText(
                                ...
                                (checkboxShowIDs.v and objectid 
                                and "model: " .. sampObjectModelNames[model] .. "(" .. model .. ") ; id: " .. objectid
                                or "model: " .. sampObjectModelNames[model] .. "(" .. model .. ")"),
                                x1, y1, -1
                            )
                        end
                        if traser.v then 
                    ...
                    elseif checkboxRenderByRule.v and sampObjectModelNames[model] ~= nil then
                        for _, v2 in ipairs(sortedObjectsTable) do
                            if v2[1].v == model and v2[3].v then
                                if checkboxRenderDistanceByRule.v then 
                                    renderFontDrawText(
                                        font,
...
function imgui.OnDrawFrame()
    ...
    if checkboxRenderAll.v then
        checkboxRenderByRule.v = false
        imgui.SameLine()
        imgui.Checkbox("Show id", checkboxShowIDs)
        imgui.SameLine()
        imgui.Checkbox("Show distance", checkboxShowDistance)
    end
    ...
    if checkboxRenderByRule.v then
        imgui.SameLine()
        imgui.Checkbox("Show id", checkboxRenderIDsByRule)
        imgui.SameLine()
        imgui.Checkbox("Show distance", checkboxRenderDistanceByRule)
    end
    ...
    if imgui.Button("Delete") then
        if removeObject.v ~= nil and removeObject.v ~= 0 then
            for _, v in pairs(getAllObjects()) do
                local model = getObjectModel(v)
                if model == removeObject.v then
                    deleteObject(v)
                end
            end
        end
    ...
    for i, value in ipairs(sortedObjectsTable) do
        ...
        if imgui.InputInt("##input" .. i, value[1], 0, -1) then
            ...
            if sampObjectModelNames[value[1].v] ~= nil then
               value[2].v = tostring(sampObjectModelNames[value[1].v])
            end 
            inicfg.save(sortedObjectsData, "renderobjects")
        end
		imgui.PopItemWidth()
        ...
        imgui.PopItemWidth()
        imgui.SameLine()
        imgui.HelpMarker("Object model name")
        imgui.SameLine()
        if imgui.Button(" x ##delbutton" .. i) then
            table.remove(sortedObjectsTable, i)
            table.remove(sortedObjectsData, i)
            inicfg.save(sortedObjectsData, "renderobjects")
        end
    end	
    imgui.EndChild()

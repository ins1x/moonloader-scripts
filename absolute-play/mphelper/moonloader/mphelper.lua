...
local input = {
   ...
   mpscore = imgui.ImInt(0),
   worldnumber = imgui.ImInt(10),
...
local checkbox = {
   ...
   sidebarvehicles = imgui.ImBool(false),
...
for id = 1, 1000 do -- 1000 == maxPlayers
   table.insert(mpScore, id, 0)
end 
...
function main()
   ...
   while true do
      ...
      if checkbox.sidebarvehicles.v then
         local vehiclesTotal = 0
         
         renderFontDrawText(sidebarfont, "Streamed vehicles:", ini.sidebar.x, ini.sidebar.y, 0xFFFFFFFF)
         ...
         for k, v in ipairs(getAllVehicles()) do
            ...
            if res then
               local line = string.format("{CDCDCD}%d(%s) {FF0000}%d {FFFFFF}%s(%d)", id, vehmodelname, health, sampGetPlayerNickname(pid), pid)
               renderFontDrawText(sidebarfont, line, ini.sidebar.x, ini.sidebar.y + (vehiclesTotal*15), 0xFFFFFFFF)
            else
               local line = string.format("{CDCDCD}%d(%s) {FF0000}%d", id, vehmodelname, health)
               renderFontDrawText(sidebarfont, line, ini.sidebar.x, ini.sidebar.y + (vehiclesTotal*15), 0xFFFFFFFF)
            end
         end
      end
...
function imgui.OnDrawFrame()
   ...
   if dialog.main.v then
      ...
      if tabmenu.header == 1 then
         ...
         if tabmenu.mp == 1 then
            ...
            -- imgui.SameLine()
            -- if imgui.TooltipButton(u8"Подсчет баллов", imgui.ImVec2(120, 25), u8"Панель для подсчета баллов в мини-играх") then
               -- dialog.mpscore.v = not dialog.mpscore.v
            -- end
         ...
         elseif tabmenu.mp == 2 then   
            ...
            if bTargetResult then
               imgui.TextColoredRGB(string.format("Позиция метки на карте {696969}x: %.1f, y: %.1f, z: %.1f",
               bX, bY, bZ))
               if imgui.IsItemClicked() then
            ...
            if imgui.TooltipButton(u8"Выбрать интерьер", imgui.ImVec2(135, 25), u8"Выбрать интерьер из списка доступных") then
               ...
               elseif isAbsolutePlay then
                  sampAddChatMessage("[SCRIPT]: {FFFFFF}Доступно только админам", 0x0FF6600) 
               else
            ...
            if imgui.TooltipButton(u8"Настр. спавн", imgui.ImVec2(100, 25), u8"Настроить спавн игроков") then
               if isTraining then
                  ...
                  dialog.main.v = false
               elseif isAbsolutePlay then
                  sampAddChatMessage("[SCRIPT]: {FFFFFF} N - Настройки - точка появления", 0x0FF6600)
               else
                  sampSendChat("/setspawn")
               end
            end
            if isAbsolutePlay then
               imgui.Text(u8"Номер мира: ")
               imgui.SameLine()
               imgui.PushItemWidth(100)
               if imgui.InputInt("##worldnumber", input.worldnumber) then
               end
               imgui.PopItemWidth()
               imgui.SameLine()
               if imgui.TooltipButton(u8"Номер мира в чат", imgui.ImVec2(120, 25), u8"Сообщить номер мира в котором проводится МП") then
                  sampSetChatInputEnabled(true)
                  sampSetChatInputText("Заходите на мероприятие в мир №"..input.worldnumber.v)
                  dialog.main.v = false
               end
            end

            imgui.Spacing()
            imgui.Text(u8"Сообщения в чат:")
            if imgui.TooltipButton(u8"Меняю спавн", imgui.ImVec2(130, 25), u8"Сообщить о смене спавна игрокам") then
               sampSetChatInputEnabled(true)
               if isAbsolutePlay then
                  sampSetChatInputText("/мчат Меняю спавн. Всем приготовиться!")
               else
                  sampSetChatInputText("Меняю спавн. Всем приготовиться!")
               end
               dialog.main.v = false
            end
            imgui.SameLine()
            if imgui.TooltipButton(u8"Занимайте позиции", imgui.ImVec2(130, 25), u8"Предупреждение игрокам") then
               sampSetChatInputEnabled(true)
               if isAbsolutePlay then
                  sampSetChatInputText("/мчат Занимайте позиции, скоро начинаем!")
               else
                  sampSetChatInputText("Меняю Занимайте позиции, скоро начинаем!")
               end
               dialog.main.v = false
            end
             
            if imgui.TooltipButton(u8"Текущее местоположение в чат", imgui.ImVec2(220, 25), u8"Сообщить ваше текущее местоположение") then
               sampSetChatInputEnabled(true)
               local positionX, positionY, positionZ = getCharCoordinates(playerPed)
         ...
         elseif tabmenu.mp == 3 then
            ...
            imgui.Text(u8"Сообщения капитанам:")
            if imgui.TooltipButton(u8"Кто хочет", imgui.ImVec2(100, 25), u8"Кто хочет быть капитаном команды?") then
               sampSetChatInputEnabled(true)
               if isAbsolutePlay then
                  sampSetChatInputText("/мчат Кто хочет быть капитаном команды?")
               else
                  sampSetChatInputText("Кто хочет быть капитаном команды?")
               end
               dialog.main.v = false
            end
            imgui.SameLine()
            if imgui.TooltipButton(u8"PvP или /try", imgui.ImVec2(100, 25), u8"За право первого выбора играем PvP или /try?") then
               sampSetChatInputEnabled(true)
               sampSetChatInputText("За право первого выбора играем PvP или /try?")
               dialog.main.v = false
            end
            imgui.Text(u8"Сообщения командам:")
            if imgui.TooltipButton(u8"Все в строй", imgui.ImVec2(100, 25), u8"Все в строй! Не разбегаемся") then
               sampSetChatInputEnabled(true)
               if isAbsolutePlay then
                  sampSetChatInputText("/мчат Все в строй! Не разбегаемся")
               else
                  sampSetChatInputText("Все в строй! Не разбегаемся")
               end
               dialog.main.v = false
            end
            imgui.SameLine()
            if imgui.TooltipButton(u8"На базу", imgui.ImVec2(100, 25), u8"Всем вернуться на базу") then
               sampSetChatInputEnabled(true)
               if isAbsolutePlay then
                  sampSetChatInputText("/мчат Всем вернуться на базу!")
               else
                  sampSetChatInputText("Всем вернуться на базу")
               end
               dialog.main.v = false
            end
            imgui.SameLine()
            if imgui.TooltipButton(u8"На исходные", imgui.ImVec2(100, 25), u8"Всем вернуться на исходные позиции") then
               sampSetChatInputEnabled(true)
               if isAbsolutePlay then
                  sampSetChatInputText("/мчат Всем вернуться на исходные позиции!")
               else
                  sampSetChatInputText("Всем вернуться на исходные позиции")
               end
               dialog.main.v = false
            end
            imgui.Text(u8"Списки:")
            if imgui.Button(u8"< Список участников команд", imgui.ImVec2(220, 25)) then
               dialog.setteams.v = not dialog.setteams.v
            end
            

            imgui.SameLine()
            if imgui.Button(u8"Все списки >", imgui.ImVec2(100, 25)) then
               tabmenu.header = 2
               tabmenu.checker = 2
               tabmenu.manage = 2 
            end
         ...
         elseif tabmenu.mp == 4 then
            ...
            if checkbox.donators.v then
               ...
               imgui.SameLine()
               if imgui.TooltipButton(u8"Объявить", imgui.ImVec2(80, 25), u8"Объявить спонсоров") then
         ...
         if tabmenu.manage == 1 then
            ...
            if tabmenu.actions == 1 then 
               ...
               if imgui.Button(u8"Установить HP", imgui.ImVec2(150, 25)) then
                  if isAbsolutePlay then
                     sampAddChatMessage("[SCRIPT]: {FFFFFF}N - Оружие - Здоровье по умолчанию", 0x0FF6600)
                  else
            ...
            elseif tabmenu.actions == 2 then
               ...
               imgui.PopItemWidth()
               imgui.SameLine()
               imgui.TextQuestion("( ? )", u8"Укажите ID либо имя транспорта для создания")
               ...
               imgui.Spacing()
               imgui.Text(u8"Управление т/c:")
               imgui.SameLine()
               imgui.PushItemWidth(50)
               if imgui.InputText("ID##VehicleIDBuffer", textbuffer.vid, imgui.InputTextFlags.CharsDecimal) then
               end
               imgui.PopItemWidth()
               imgui.SameLine()
               imgui.TextQuestion("( ? )", u8"Укажите ID транспорта для управления")
               ...
               imgui.Spacing()
               if imgui.Button(u8"Удалить т/c", imgui.ImVec2(120, 25)) then
                  if vid then
            ...
            elseif tabmenu.actions == 3 then
               ...
               if imgui.Button(u8"Уведомить о пополнении игроков", imgui.ImVec2(250, 25)) then
                  sampSetChatInputEnabled(true)
                  if isTraining then
                     sampSetChatInputText('@ Сейчас будут пополнены хп и броня для всех игроков!')
                  else
                     sampSetChatInputText('Сейчас будут пополнены хп и броня для всех игроков!')
                  end
               end
               ...
               if imgui.Button(u8"Уведомить о спавне транспорта", imgui.ImVec2(250, 25)) then
                  sampSetChatInputEnabled(true)
                  if isTraining then
                     sampSetChatInputText('@ Респавн транспорта. Займите транспорт иначе он пропадет!')
                  else
                     sampSetChatInputText('@ Респавн транспорта. Займите транспорт иначе он пропадет!')
                  end
               end
               if imgui.Button(u8"Починить весь транспорт", imgui.ImVec2(250, 25)) then
                  if isAbsolutePlay then
               ...
               if imgui.Button(u8"Зареспавить весь транспорт", imgui.ImVec2(250, 25)) then
                  if isAbsolutePlay then
                     sampSendChat("/spcars")
                  else
                     sampAddChatMessage("Недоступно для вашего сервера.", -1)
         ...
         elseif tabmenu.manage == 2 then
            ...
            elseif tabmenu.checker == 2 then
               ...
               if imgui.Checkbox(u8("Боковая панель со списком игроков в стриме"), checkbox.sidebarplayers) then
                  ...
                  -- inicfg.save(ini, configIni)
                  if checkbox.sidebarvehicles.v then
                     checkbox.sidebarvehicles.v = false
                  end
               end
               
               if imgui.Checkbox(u8("Боковая панель со списком транспорта в стриме"), checkbox.sidebarvehicles) then
                  if checkbox.sidebarplayers.v then
                     checkbox.sidebarplayers.v = false
                  end
               end
               
               imgui.Text(u8"Проверить игроков:")
	           if imgui.Button(u8"Вывести список лагеров", imgui.ImVec2(200, 25)) then
	              local counter = 0
	 	          if next(playersTable) == nil then -- if playersTable is empty
	 	             sampAddChatMessage("Сперва обнови список игроков!", -1) 
	 	          else
               ...
               imgui.SameLine()
	           if imgui.Button(u8"Вывести список игроков AFK", imgui.ImVec2(200, 25)) then
	              local counter = 0
	 	          if next(playersTable) == nil then -- if playersTable is empty
	 	             sampAddChatMessage("Сперва обнови список игроков!", -1) 
	 	          else
               ...
               if imgui.Button(u8"Статистика всего онлайна", imgui.ImVec2(200, 25)) then
                  local totalonline = 0
	 	          local olds = 0
                  local players = 0
	 	          local newbies = 0
               ...
               imgui.SameLine()
               if imgui.Button(u8"Черный список игроков", imgui.ImVec2(200, 25)) then
               ...
               if imgui.Button(u8"Игроки с оружием", imgui.ImVec2(200, 25)) then
               ...
               imgui.SameLine()
               if imgui.Button(u8"Игроки с малым уровнем", imgui.ImVec2(200, 25)) then
                  local minscore = 5
                  local noobs = 0
      ...
      elseif tabmenu.header == 3 then
         ...
         if tabmenu.presets == 1 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.GetStyle().Colors[imgui.Col.ButtonHovered])
            if imgui.Button(u8"Бинд-лист", imgui.ImVec2(75, 30)) then tabmenu.presets = 1 end
            imgui.PopStyleColor()
         else
            if imgui.Button(u8"Бинд-лист", imgui.ImVec2(75, 30)) then tabmenu.presets = 1 end
         end
         ...
         if tabmenu.presets == 2 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.GetStyle().Colors[imgui.Col.ButtonHovered])
            if imgui.Button(u8"Чат-игры", imgui.ImVec2(75, 30)) then tabmenu.presets = 2 end
            imgui.PopStyleColor()
         else
            if imgui.Button(u8"Чат-игры", imgui.ImVec2(75, 30)) then tabmenu.presets = 2 end
         end
         ...
         if tabmenu.presets == 3 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.GetStyle().Colors[imgui.Col.ButtonHovered])
            if imgui.Button(u8"Правила", imgui.ImVec2(75, 30)) then tabmenu.presets = 3 end
            imgui.PopStyleColor()
         else
            if imgui.Button(u8"Правила", imgui.ImVec2(75, 30)) then tabmenu.presets = 3 end
         end
      ...
      elseif tabmenu.header == 4 then
         ...
         imgui.Text(u8"Работает на проектах")
         ...
         imgui.TextColoredRGB("{696969}Скрипт распостраняется только с открытым исходным кодом.")
		 ...
		 imgui.SameLine()
		 imgui.Link("https://github.com/ins1x/mphelper/wiki","https://github.com/ins1x/mphelper/wiki")
         ...
         if imgui.Button(u8"Проверить обновления",imgui.ImVec2(170, 25)) then
            if not checkScriptUpdates() then
               sampAddChatMessage("{696969}MP Helper  {FFFFFF}Установлена актуальная версия {696969}"..thisScript().version, -1)
   ...
   if dialog.playerstat.v then
      ...
      if imgui.TooltipButton(u8"Пробить игрока", imgui.ImVec2(220, 25), u8"Получить информацию о игроке через сайт (онлайн)") then
         if isTraining then
            os.execute(string.format('explorer "https://training-server.com/api/user/%s"', nickname))
         elseif isAbsolutePlay then
            os.execute(string.format('explorer "https://gta-samp.ru/server-deathmatch?Nick=%s"', nickname))
         end
         dialog.main.v = false
      end
   ...
   if dialog.setteams.v then
      ...
      imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
      imgui.SetNextWindowSize(imgui.ImVec2(300, 360))
      ...
      if imgui.Button(u8"Экспортировать", imgui.ImVec2(125, 25)) then
         local file = io.open("moonloader/resource/mphelper/teams.txt", "w")
         local countTeamA = 0
         local countTeamB = 0
         file:write("Team A:\n")
         ...
         file:write(string.format("players: %d\n", countTeamA))
         file:write("\n")
         file:write("Team B:\n")
         ...
         file:write(string.format("players: %d\n", countTeamB))
         file:write("\n")
         file:write(string.format("Total players: %d\n", countTeamA+countTeamB))
         file:close()
         sampAddChatMessage("Таблица игроков сохранена.", -1)
      end
      imgui.End()
   end
   
   if dialog.mpscore.v then
      imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 6, sizeY / 2),
      imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
      imgui.SetNextWindowSize(imgui.ImVec2(300, 350))
      imgui.Begin(u8"Подсчет баллов", dialog.mpscore)
      
      imgui.Text(u8"Подсчет баллов:")
      
      if imgui.Button(u8"Сбросить счет", imgui.ImVec2(100, 25)) then
      end
      imgui.SameLine()
      if imgui.Button(u8"Вывести в чат", imgui.ImVec2(100, 25)) then
      end
      ...
      if imgui.Button(u8"Добавить игрока", imgui.ImVec2(150, 25)) then
         for key, value in ipairs(mpScore) do
            if tonumber(key) == tonumber(pid) then
               sampAddChatMessage("Этот игрок уже был добавлен", -1)
               break
            end
            table.insert(mpScore, tonumber(pid), 1)
            sampAddChatMessage(string.format("В таблицу добавлен игрок %s(%d)", 
            sampGetPlayerNickname(pid), pid), -1)
         end
      end
      
      imgui.Text(u8"Кол-во баллов:")
      for key, value in ipairs(mpScore) do
         if value > 0 then
            imgui.Text(string.format("%s(%d) - score", sampGetPlayerNickname(tonumber(key)), tonumber(key), value))
         end
         -- imgui.PushItemWidth(150)
         -- imgui.InputInt("##inputmpscore", input.mpscore)
         -- imgui.PopItemWidth()
      end
...
function sampev.onSendEnterVehicle(vehicleId, passenger)
   textbuffer.vid.v = tostring(vehicleId)
end
...
function checkScriptUpdates()
   if doesFileExist(getGameDirectory() .. "\\moonloader\\lib\\requests.lua") then
      local result, response = pcall(require('requests').get, 'https://raw.githubusercontent.com/ins1x/mphelper/main/version.dat')
      if result then
         if response.status_code == 200 then
            ...
            if tonumber(version) > tonumber(installedversion) then
               sampAddChatMessage("{696969}MP Helper  {FFFFFF}Доступно обновление до версии {696969}"..text, -1)
               return true
            end
         else
            print("MP Helper: Check updates failed server not responded")
            return false
         end
      else
         print("MP Helper: Check updates failed server unavailable")
         return false
      end
   else
